...
from prosperity_lib.tools.data_reader import BackDataReader, PackageResourcesReader, FileSystemReader
...
from prosperity_lib.models.test_options import TestOptions, RoundDayOption
from prosperity_lib.test_runner import TestRunner
from prosperity_lib.tools.visualizer import Visualizer
...
class BackTester:
    ...
    def __get_data_reader(self, data_dir) -> BackDataReader:
        if data_dir is not None:
            absolute_path = os.path.abspath(data_dir)
            # We finally have the class to handle the path!
            return FileSystemReader(absolute_path)
        else:
