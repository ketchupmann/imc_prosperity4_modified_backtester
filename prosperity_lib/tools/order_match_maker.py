import math
import random
from prosperity_lib.datamodel import TradingState, Order, Symbol, Trade
from prosperity_lib.models.input import BacktestData, MarketTrade
from prosperity_lib.models.output import TradeRow
from prosperity_lib.models.test_options import TradeMatchingMode
...
class OrderMatchMaker:
    ...
    def __simulate_probabilistic_fill(self, order: Order) -> bool:
        depth = self.state.order_depths.get(order.symbol)
        ...
        # Calculate Imbalance (using abs() for Prosperity's negative sell volumes)
        vol_bid = sum(depth.buy_orders.values())
        vol_ask = abs(sum(depth.sell_orders.values()))
        
        total_vol = vol_bid + vol_ask
        if total_vol == 0: 
            total_vol = 1
            
        imbalance = vol_bid / total_vol

        # Calculate Distance Penalty
        distance = abs(mid_price - order.price)
        ...
        if order.quantity < 0: # SELL order
            ...
            adjusted_prob = base_prob * ((1 - imbalance) * 2) * distance_penalty
